...
if (!loadedVersions.includes('/js/frames/versionNyxFlavorSaga.js')) {
	loadedVersions.push('/js/frames/versionNyxFlavorSaga.js');
	sizeCanvas('saga');
...
function sagaEdited() {
	...
	for (var i = 0; i < card.saga.count; i ++) {
		...
		if (sagaChapter.complete) {
			...
			var numeralTextX = numeralX + scaleWidth(0.0594);
